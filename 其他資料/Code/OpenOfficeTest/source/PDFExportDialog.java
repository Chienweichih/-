...
public class PDFExportDialog
{
	...
	public void run()
	{
		...
		// check them all
		if (xExecutableDialog == null || xExporter == null
				|| xPropertyAccess == null)
		{
		...
		try
		{
			...
			PropertyValue[] aPropertyValues = xPropertyAccess
					.getPropertyValues();
			...
			if (aFilterData != null)
			{
				...
				String sURL = getHomeDir(m_xContext)
						+ "/PDF_EXPORT_DIALOG_demo.pdf";
				// choose the proper filter
				String sFilter = "writer_pdf_Export";

				boolean bExport = doExport(xComponent, sURL, sFilter,
						aFilterData);
